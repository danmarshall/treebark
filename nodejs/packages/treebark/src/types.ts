...
// Definition for a custom (userland) tag. Two mutually-compatible expansion
// modes are supported (a definition must provide at least one):
// - `expand` ("Tier 1" — template expansion): receives already name-validated
//   (but not yet interpolated) attributes and the tag's raw, unrendered children,
//   and must return a template built only from built-in tags/special keys (which
//   may itself reference other custom tags). The returned template is rendered
//   through the normal pipeline, so all existing interpolation, escaping, and
//   attribute-value validation still applies to it. Supported by every renderer
//   (string, DOM, React).
// - `component` ("Tier 2" — React component override): only honored by the React
//   renderer (`renderToReact`/`<Treebark>`); ignored by the string/DOM renderers,
//   which fall back to `expand` (an error is logged if `expand` is missing there).
//   Typed loosely here (renderer-agnostic file) — see `ReactCustomTagDefinition`
//   in `react.ts` for the properly-typed React component signature.
...
//   always allowed. Attributes not in either set are stripped before `expand`/
//   `component` runs.
