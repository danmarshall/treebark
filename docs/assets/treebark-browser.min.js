...
`);return o+=`
`,o};function M(e,n={}){const o=e.data,r=n.logger||console,i=n.indent?{indentStr:typeof n.indent=="number"?" ".repeat(n.indent):typeof n.indent=="string"?n.indent:"  ",level:0,logger:r}:{logger:r};return h(e.template,o,i)}function V(e,n,o,r,i,s,t,c=[]){const l=F(r,s),u=l.startsWith(`
`)&&s?s.repeat(t||0):"";if(e==="$comment")return`<!--${l}${u}-->`;const f=`<${e}${Y(n,o,e,c,i)}>`;return y.has(e)?f:`${f}${l}${u}</${e}>`}function h(e,n,o){const r=o.parents||[],i=o.logger;if(typeof e=="string")return A(e,n,!0,r,i);if(Array.isArray(e))return e.map(a=>h(a,n,o)).join(o.indentStr?`
`:"");const s=K(e,i);if(!s)return"";const{tag:t,rest:c,children:l,attrs:u}=s;if(!D.has(t))return i.error(`Tag "${t}" is not allowed`),"";if(t==="$comment"&&o.insideComment)return i.error("Nested comments are not allowed"),"";if(t==="$if"){const{valueToRender:a}=U(c,n,r,i);return a===void 0?"":h(a,n,o)}y.has(t)&&l.length>0&&i.warn(`Tag "${t}" is a void element and cannot have children`);const f={...o,insideComment:t==="$comment"||o.insideComment,level:(o.level||0)+1},j=a=>a===""?[]:o.indentStr&&a.includes(`
`)&&!a.includes("<")?a.split(`
`).map(S=>[f.level,S]):[[f.level,a]];let d,T;if(N(c)){if(!b(c.$bind,"$bind",i))return"";const a=m(n,c.$bind,[],i),{$bind:S,$children:E=[],...I}=c;if(!Array.isArray(a)){if(a!=null&&typeof a!="object")return i.error(`$bind resolved to primitive value of type "${typeof a}", cannot render children`),"";const w=a&&typeof a=="object"&&a!==null?a:{},g=[...r,n];return h({[t]:{...I,$children:E}},w,{...o,parents:g})}if(d=[],!y.has(t))for(const w of a){const g=[...r,n];for(const H of E){const J=h(H,w,{...f,parents:g});d.push(...j(J))}}T=I}else{if(d=[],!y.has(t))for(const a of l){const S=h(a,n,{...f,parents:r});d.push(...j(S))}T=u}return V(t,T,n,d,i,o.indentStr,o.level,r)}function Y(e,n,o,r=[],i){const s=Object.entries(e).filter(([t])=>z(t,o,i)).map(([t,c])=>{let l;if(t==="style"){if(l=G(c,n,r,i),!l)return null}else if(q(c)){const u=B(c,n,r,i);l=A(String(u),n,!1,r,i)}else l=A(String(c),n,!1,r,i);return`${t}="${k(l)}"`}).filter(t=>t!==null).join(" ");return s?" "+s:""}$.renderToString=M,Object.defineProperty($,Symbol.toStringTag,{value:"Module"})}));
